...
- [ ] 从已审核提交生成固定版本标签、Release 说明和镜像；记录支持范围、升级步骤、已知限制及校验信息，不附入运行数据。

### GitHub 仓库设置

- [ ] 补充仓库 Description、Topics 与文档入口；当前不假设公开仓库地址或可直接拉取的官方镜像已存在。
...
- [ ] 核对每周依赖审计及 Dependabot 更新；本地新锁文件审计通过，远端 CI 状态须在提交后核实，不得忽略新公告或允许失败。CI 不自动推送镜像、创建 Release 或部署。
